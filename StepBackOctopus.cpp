...
StepBackOctopus::StepBackOctopus(Panel* container, GridOctopus* gridO, Spell* s, Character* caster, SpellTarget* target)
...
{
	Texture t = (*ServiceLocator::getTextureManager())["ArrowArcher"];
	_tex = t;
...
StepBackOctopus::~StepBackOctopus()
{
}


void StepBackOctopus::update()
{
	if (isActive())
	{
...
void StepBackOctopus::internalRender(SDL_Renderer* r, bool force)
{
	if (isActive())
	{
